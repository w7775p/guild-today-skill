...
Result 保存后台事实与结算后果。最终报告正文与信息组织归报告与情报揭露系统。
...
Event 的后台事实与即时玩家反馈分开维护。即时玩家反馈需要实际信息渠道；需要玩家选择的 Event 需要实际参与渠道。
...
当前垂直切片不引入正式 TimeSystem、日历或每日刷新。只有真实 Task 出现固定日程、特殊周期或稳定时间引用需求时读取并建立 Time 依赖。
